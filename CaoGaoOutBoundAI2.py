...
class UserBehavior():
    ...
    def handle(self):
        ...
        ### 改动点三 ###: 仅用于调试 False-调试 True-忽略，默认True
        debug = True
        # with self.client.post(url=uri, headers=self.configHeader(data), json=data) as response:
        #     # 设置断言，检查响应状态码是否为 200
        #     assert response.status_code == 200, "服务器错误：服务器返回了非2xx 的状态码（例: 404 Not Found、" \
        #                                         "500 Internal Server Error 等），请检查咨询开发是否部署，域名、环境是否正确等"
        #     # 检查接口响应状态码是否为 0
        #     assert response.json()[
        #                "code"] == 0, f"接口状态码错误：请检查参数、域名、环境等； response：{response.json()} request：{data}"
        #
        #     # 仅用于调试
        #     assert debug, "仅用于调试【正式压测时，请关闭调试】:" + response.text
